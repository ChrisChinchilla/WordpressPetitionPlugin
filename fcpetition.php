<?php
/*
Plugin Name: WordPress Petition
Description: Simple petitions with e-mail based confirmation to your WordPress installation.
Version: 1
Based on FreeCharity.org.uk Wordpress Petition by James Davis, Free Charity
Adapted by Tim Norton, Oxfam Australia
*/
?>
<?php
...
$options_defaults = array (
	"petition_title" 		=> '',
	"petition_text"  		=> __("TEXT HERE","fcpetition"),
	"petition_confirmation"	=> __("Thank you for taking action.\n\nTo ensure your voice is heard, without being lost in a sea of spam, please confirm this by visiting the following address:\n\n[[curl]]","fcpetition"),
	"petition_confirmurl" 	=> __("<PLEASE ENTER THE CORRECT URL>","fcpetition"),
	"petition_from" 		=>  sprintf(__("MPH Petitions <%s>","fcpetition"),get_option('admin_email')),
	"petition_cc"			=> '',
	"petition_cc_subject"	=> '',
	"petition_cc_text"		=> '',
	"petition_maximum" 		=> 10,
...
/*
 *
 *  Disable e-mail verification of petitions.
 *  THIS IS A BAD THING. ENABLING THIS FEATURE WILL OPEN YOUR PETITION TO ABUSE AND SPAM.
 *  Set the option to 1 if you really want this.
 */ 
define("OVERRIDE_VERIFICATION",0);
/* 
 *
 *  Allow the administrator to manually confirm signatures. 
 *  Set the option to 1 if you really want to do this.
 */
define("OVERRIDE_CONFIRMATION",1);
...
$petitions_table_sql = "CREATE TABLE $petitions_table (
						...
						`petition_cc` VARCHAR(100),
						`petition_cc_subject` VARCHAR(100),
						`petition_cc_text` TEXT,
...
function fcpetition_widget($args){
	...
	echo $before_title . __("Total Petition Signatures","fcpetition") . $after_title; 
		...
		$sql = "SELECT count($signature_table.email) as count,petition_title FROM $signature_table,$petitions_table where $signature_table.petition = $petitions_table.petition and $signature_table.confirm = '' and petition_enabled = 1;";
...
*/
function fcpetition_returncount()
{
	$sql = "SELECT count($signature_table.email) as count,petition_title FROM $signature_table,$petitions_table where $signature_table.petition = $petitions_table.petition and $signature_table.confirm = '' and petition_enabled = 1 group by petition_title;";
 ...
 */
function fcpetition_confirm($confirm = ''){
	global $wpdb;
	global $signature_table;
	global $petitions_table;
...
$noTemplate = 1;
	...
	$rs = $wpdb->get_results("SELECT * FROM $signature_table WHERE confirm = '$confirm' AND cc = 'yes_cc'") ;

	if ($wpdb->query("UPDATE $signature_table SET `confirm` = '' WHERE `confirm` = '$confirm'")==1) {

	$htmlReturn = '<p>Thank you for being a voice for Australias lifeblood: the Murray-Darling. Together, we can return the rivers of the Murray-Darling Basin to health and secure the future of people who live and work there.</p> 

<p>Here are a few additional things you can do as a Voice for the Murray-Darling</p>
<ul>
<li>ask your friends to become a Voice for the Murray-Darling by signing the petition at <a href="http://lifeblood.org.au" target="_blank">www.lifeblood.org.au</a>
<li><a href="http://lifeblood.org.au/wp-content/uploads/2011/05/Petition-offline.pdf">download and print the petition</a> and get others from your school, your football club or your community to add their voice
<li>Share your Murray-Darling story for the testimonial section of the website by sending us an email at <a href="mailto:info@lifeblood.org.au">info@lifeblood.org.au</a></ul>' ;

	
	$petition = $rs[0]->petition;


		$email = $wpdb->escape($rs[0]->email);
		$email =  htmlchars(wp_kses($email,array()));
						...
						{	
   							...
   							$headers = 'cc: Tony Burke <Tony.Burke.MP@aph.gov.au>' . "\r\n";
							fcpetition_ccmail($email,$petition,$headers);
...
{
$htmlReturn = "The confirmation code you supplied was invalid. Either it was incorrect or it has already been used.";
}
...
if ($noTemplate != 1)
{
	...
	<?php get_header(); ?>
		...
		<h2><?php printf(__('Thanks for confirming your petition signature - %s', "fcpetition"), get_bloginfo('name')); ?></h2>
		<p>
	<?php
		print __($htmlReturn,"fcpetition");

	?>
		</p>
        <p>
        <a href="<?php bloginfo('home')?>/signatures">View current signatories</a>
</p>
		<p>
...
function fcpetition_filter_pages($content) {
    ...
    $petition = wp_kses($petition,array());
    	...
    	$refer = $wpdb->escape($_POST['refer']);
    $refer = wp_kses($refer,array());
	...
	if( $_POST['petition_posted'] == 'Y' && preg_match("/\[\[petition-$petition\]\]/",$content)) {
		...
		$wpdb->hide_errors();
		if ($name == ""){
			return __("Sorry, you must enter a name.","fcpetition");
		...
		} elseif ($wpdb->query("INSERT INTO $signature_table (`petition`,`email`,`name`,`confirm`,`comment`,`time`,`fields`,`keep_private`,`refer`,`cc`) VALUES ('$petition','$email','$name','$confirm','$comment',NOW(),'$fields','$keep_private','$refer','$cc')")===FALSE){
			# This has almost certainly occurred due to a duplicate email key
                        $wpdb->show_errors();
                        //return __("Sorry, someone has already used this e-mail address.","fcpetition");
return __("<div class='petition'><blockquote>Sorry, someone has already used this e-mail address.</blockquote></div>","fcpetition");
		} else {
			$wpdb->show_errors();
                        # Successful signature, send an e-mail asking the user to confirm
							...
							return __("<div class='petition'><blockquote>Thank you for signing the petition to protect Australia’s lifeblood the Murray-Darling. Together we can return the Murray-Darling to health and secure the future of people who live and work in the Basin by permanently ending the over-use of water.<p><b>To prevent spam we have just sent you an email to confirm your address. Please make sure you check your inbox (it could also be in Spam) otherwise your name may not be added.</b></p><p>Will you ask your friends to join the growing chorus of voices for the Murray-Darling?</p><p>You can view current signatories <a href='/signatures'>here</a>.</blockquote></div>","fcpetition");		
                        	...
                        	//return __("Thank you for taking action. To avoid spam, we've sent you a confirmation e-mail. Please check your inbox and click the confirmation link.","fcpetition");
			return __("<div class='petition'><blockquote>Thank you for signing the petition to protect Australia's lifeblood the Murray-Darling. Together we can return the Murray-Darling to health and secure the future of people who live and work in the Basin by permanently ending the over-use of water.<p>To prevent spam we have just sent you an email to confirm your address.</p><p>Will you ask your friends to join the growing chorus of voices for the Murray-Darling?</p><p>You can view current signatories <a href='/signatures'>here</a>.</blockquote></div>","fcpetition");		
...
function fcpetition_mail($email,$po){
	...
	$petition_confirmation = stripslashes($rs[0]->petition_confirmation);
	...
	$petition_confirmation = str_replace('[[curl]]',$confirm_url,$petition_confirmation);
	$subject = sprintf(__("Thank you for signing the Lifeblood petition!","fcpetition"),$petition_title);
	wp_mail($email,"$subject","$petition_confirmation","From: $petition_from");
}

function fcpetition_ccmail($email,$po,$headers){
	global $wpdb;
	global $signature_table;
	global $petitions_table;
	$rs = $wpdb->get_results("SELECT `petition_cc`,`email`, `keep_private`, `name`,`petition_title`,`petition_cc_text`,`petition_cc_subject` from $signature_table,$petitions_table where $petitions_table.petition = $signature_table.petition and `email` = '$email' and $petitions_table.petition = '$po';");
	$petition_from = stripslashes($rs[0]->email);
	$name = stripslashes($rs[0]->name);


if ($rs[0]->keep_private == 'on') {
				$name = "Anonymous";
			} else {
				$name = $name;
			}

$full_headers = $headers . "\r\nFrom: ".$name. " <" . $petition_from . ">" ;





	$email = stripslashes($rs[0]->petition_cc);

	$petition_title = stripslashes($rs[0]->petition_title);

	$email_body = stripslashes($rs[0]->petition_cc_text);
$email_final_body = $email_body . $name;
	$subject = stripslashes($rs[0]->petition_cc_subject);
	wp_mail($email,"$subject","$email_final_body",$full_headers);
}
/*
function fcpetition_followmail($email,$po){
	global $wpdb;
	global $signature_table;
	global $petitions_table;
	$rs = $wpdb->get_results("SELECT `petition_cc`,`email`,`petition_title`,`petition_cc_text`,`petition_cc_subject` from $signature_table,$petitions_table where $petitions_table.petition = $signature_table.petition and `email` = '$email' and $petitions_table.petition = '$po';");
	$petition_from = stripslashes($rs[0]->petition_from);
	//$email = stripslashes($rs[0]->petition_cc);
	$petition_title = stripslashes($rs[0]->petition_title);

	$email_body = stripslashes("");
	$subject = sprintf(__("Thank you for taking action in the '%s'","fcpetition"),$petition_title);
	wp_mail($email,"$subject","$email_body","From: $petition_from");
}
*/
/*
...
function fcpetition_form_top($petition,$action){
	...
	$name = __("Name (First name and Last name)","fcpetition");
	$email = __("E-mail address (will be kept private)","fcpetition");
	$privacy =  __("I wish my name to remain anonymous","fcpetition");
	
		  $full_url = explode("/",$_SERVER["REQUEST_URI"]) ;
      $slug = $full_url[1];
      
	$button = __("Submit","fcpetition");
	if($comments_enabled){
		$comments_form = sprintf(__("Optional comment","fcpetition")).":<br/><textarea name='petition_comment' cols='50'></textarea><br/>";
	}
	$custom_fields =  fcpetition_livefields($petition);

    $sql = "SELECT count($signature_table.email) as count,petition_title FROM $signature_table,$petitions_table where $signature_table.petition = $petitions_table.petition and petition_enabled = 1 and $signature_table.confirm = '' ORDER BY `time` DESC;";
	$result = $wpdb->get_results($sql);

	print "<div id='sigs'><b>Total signatures</b><p> " . $result[0]->count . "</p></div>" ;
	
	return "<blockquote>
		$text
		<form name='petition' method='post' action='$action' class='petition'>
			<input type='hidden' name='petition_posted' value='Y'/><hr>
			$name:<br/><input type='text' name='petition_name' value='' size='40'/><br/>
			$email:<br/><input type='text' name='petition_email' value='' size='40'/><br/>
			$custom_fields
			$comments_form
			<input type='checkbox' name='cc' value='yes_cc' checked='true' /> cc your message to Federal Water Minister Tony Burke<br/> 
			<input type='checkbox' name='petition_keep_private' value='on' /> $privacy
			 <input type='hidden' name='petition' value='$petition'/>
			 <input type='hidden' name='refer' value='$slug'/>
			 <br/>
...
function fcpetition_form_bottom($petition) {
	...
	# You can edit the following empty string if you wish. For instance:
	...
	foreach($wpdb->get_results($sql) as $row) {
			...
			if ( $comments_enabled == 1 && $row->comment != "") {
				...
				$return  .= sprintf("<!--<p><span class='signature'><b>%s</b>, %s<br/><i>%s</i></span></p>-->",$the_name,$fields,$comment);
			} else {
				$return  .= sprintf("<!--<p><span class='signature'><b>%s</b>, %s</span></p>-->",$the_name,$fields);
			}
	}
...
function fcpetition_form($petition){
	...
	return "</p><div class='petition'>".$form1.$form2."</div>";
}
...
// REPAGINATE
	...
	$n = 0;
	$count = 50;
 $results = $wpdb->get_results("SELECT * FROM $signature_table WHERE `keep_private`!='on' and `confirm` = ''  ORDER BY `time` DESC LIMIT $n,$count"); 
		if (count($results) < 1) {
	...
	?>
		...
		<table class="widefat">
		<tr><thead><th><?php _e("Name","fcpetition"); ?></th>
<th><?php _e('Suburb, State and Postcode',"fcpetition"); ?></th><th></th></thead></tr>
		<?php
...
if ($row->keep_private == 'on') {
 ...
 echo stripslashes($the_name); ?>

</td>
				

				<td><?php fcpetition_prettyvalues(unserialize(base64_decode($row->fields)),1); // BAD DIRTY HACK ****?></td>
...
function fcpetition_manage_page() {
	...
	if($_POST['manualconfirm'] != ''){
		...
		$rs = $wpdb->get_results("SELECT * FROM $signature_table where  `email` = '$email' AND `petition`='$po'");
//print_r ($rs);
//print 'c'.$rs[0]->confirm;
		fcpetition_confirm($rs[0]->confirm);
		echo '<div id="message" class="updated fade"><p><strong>';
...
function fcpetition_livefields($po) {
	...
	if(count($res)>0) {
		foreach($res as $row){
			...
			if($row->type == "text") {
				$output .= "$row->name$lmsg:<br/><input type='$row->type' name='$row->name' size='40'/><br/>\n";
...
function fcpetition_settings_page() {
	...
	if ( $_POST['editfieldoptions']){
		...
		<p>
			<?php _e("Please enter the address which the e-mail will be sent to.","fcpetition")?><br/>
			<input type="text" name="petition_cc" value="<?php echo stripslashes($petition_cc); ?>" size="72"/>
		</p>
		
				<p>
			<?php _e("Please enter the subject of the email.","fcpetition")?><br/>
			<input type="text" name="petition_cc_subject" value="<?php echo stripslashes($petition_cc_subject); ?>" size="100"/>
		</p>
				<p>
			<?php _e("Please enter the email text","fcpetition")?><br/>
			<textarea name="petition_cc_text" rows="10" cols="72"><?php echo stripslashes($petition_cc_text); ?></textarea>
		</p>
		...
		<p>
			<?php _e("Allow signatories to leave an optional comment","fcpetition")?>
		...
		<?php } ?>
			<hr/>
			<p>Originally written by James Davis, updated by Tim Norton and licensed under the GNU GPL.
